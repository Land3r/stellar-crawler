...
export type TNodeData = Record<string, string>;
...
export const computeInfos = (data: TNodeData[], setDateFn = setDate, setDisplayFn = setDisplay) =>
  data.map(({ _id, id, name, type, address }) => ({
    key: _id,
    cols: {
      ...setDisplayFn({ id }),
      ...setDisplayFn({ name }),
      ...setDisplayFn({ type }),
      ...setDisplayFn({ address }),
      ...setDisplayFn({ id })
...
export const useNodes = ({
  ...
  const { data, isFetching, error, refetch } = useQueryFn({
    queryKey: [`nodes?max=${Number(numberItems)}&sort=${field}&dir=${order}&skip=${Number(page * numberItems)}`],
    select: computeDataQueryFn,
  });

  return {
    nodes: data?.nodes?.data ?? DEFAULT_STATE_VALUE.data,
    pagination: data?.nodes?.pagination ?? DEFAULT_STATE_VALUE.pagination,
...
export type TReturnUseNodes = ReturnType<typeof useNodes>;
